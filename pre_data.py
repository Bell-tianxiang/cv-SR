...
TEST_PATH = os.path.join('','data/Test/Set14/')
Random_Crop = 30
Patch_size = 32
label_size = 20
conv_side = 6
...
def prepare_data(_path):
    ...
    for i in range(nums):
        ...
        lr_img = cv2.resize(hr_img, (round(shape[1] / scale), round(shape[0] / scale)))
...
def prepare_crop_data(_path):
    ...
    for i in range(nums):
        ...
        hr_img = cv2.cvtColor(hr_img, cv2.COLOR_BGR2YCrCb)
        hr_img = hr_img[:, :, 0]
        hr_img = cv2.resize(hr_img, (img_size, hr_img.shape[0]))
        shape = hr_img.shape

        lr_img = cv2.resize(hr_img, (round(shape[1] / scale), round(shape[0] / scale)))
        lr_img = cv2.resize(lr_img, (img_size, shape[0]))
        # shape = lr_img.shape
